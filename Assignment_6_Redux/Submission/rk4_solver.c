...
{
	int obj;
	double *pdvsx, *pdvsy, *pmin_t, dvsx, dvsy, dvs, min_t, clearance, DT;
	DT = 100;
	dvsx = 0;
	dvsy = 50;
	obj = 2;
...
**********************Run the function*****************************************************/
//	rk4(pmin_t, pdvsx, pdvsy, clearance, obj,DT,dvsx,dvsy);
...
void rk4(double *pmin_t, double *pdvsx, double *pdvsy, double clearance, int obj, double DT, double dvsx, double dvsy)
{
	//printf("min Time: %.f, dVsx: %f, dVsy: %f\n", *pmin_t, *pdvsx, *pdvsx);
	...
	double min_dvs	;	/* Set minimum dvs */
	...
	dvs = sqrt(pow(dvsx,2)+pow(dvsy,2));	
	min_dvs = sqrt(pow(*pdvsx,2)+pow(*pdvsy,2));
	...
	// Conditional to not enter while loop if dvs is not smaller than min_dvs
	if (dvs > dms && obj == 1)
	{
			des = 3*dem; // Won't enter while loop and won't change minimums.
	}
/**********************************************************	
...
*************************************************************/	
	...
	xm = u0[6];
	ym = u0[7];
	dms = sqrt(pow((xm - xs),2) + pow((ym - ys),2));
	dem = sqrt(pow((xe - xm),2) + pow((ye - ym),2));
	des = sqrt(pow((xe - xs),2) + pow((ye - ys),2));
	if (T0 > *pmin_t)
	{
		des = 3*dem; // Will exit while loop and won't change minimums.
	}
	free ( u1 );
	}
/******************************************************************************
...
//**********************************************
	...
	b = clearance;
	c = tol;
	char filename[ sizeof "Optimum_1_10000_0p1"];
	sprintf(filename, "Optimum_%d_%d_0p%d",a,b,c);
//	printf("%s \n", filename);		
