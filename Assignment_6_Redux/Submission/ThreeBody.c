...
switch (io.OBJ)	{
	...
	case 2 :
		...
		printf("The corresponding dvsx is: %f [m/s]. \n",io.dVsx);
		printf("The corresponding dvsy is: %f [m/s]. \n",io.dVsy);
		break;
...
for ( j=0; j<21; j++ ){
	for (k=0; k<22; k++ ){		
		...
		//Run RK4		
		rk4(pmin_T, pdVsx , pdVsy, io->CLEAR, io->OBJ, DT, io->array_out[j][0] ,io->array_out[j][k+1] );
	}
...
get_solution_vectors(pmin_T, pdVsx , pdVsy, io->CLEAR, io->OBJ, DT, io->TOL);
}
