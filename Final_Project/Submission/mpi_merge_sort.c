...
//
// Description: This is the first draft of mpi merge sort C code. This code uses
...
// with the array sorted from least to greatest. final_mpi_merge_sort.c contains 
// the cleaned up version of this same code.
//
// To Compile: mpicc -o sort mpi_merge_sort.c 
...
/*************************Set up MPI********************************************/
	...
	*****************************************************************/
	...
	/* Create A and B */
	//double *A = calloc(size_per_processor, sizeof(double));
	//double *B = calloc(size_per_processor + 1, sizeof(double));
...
/*
			...
			{
				// Send to serverrank
				destination = serverrank;
				mpi_error_code = MPI_Send(A, size_per_processor, MPI_DOUBLE, destination, tag, MPI_COMM_WORLD);
			}
			else
			{
				for (i=0;i<size_per_processor;i++)
...
*/
	...
	*******************************************************************/
	...
	if (myrank == serverrank)
	{
		for (source=0; source<nprocs-rem_values; source++) // Get A arrays
		{
			if (source != serverrank)
			{
				double *A = calloc(size_per_processor, sizeof(double));
				mpi_error_code = MPI_Recv(A, size_per_processor, MPI_DOUBLE, source, tag, MPI_COMM_WORLD, &status);
				
				// Print for debugging

				for (i=0; i<size_per_processor ;i++)
				{
					//printf("The message after being recieved is: %f \n", A[i]);
					y[solution_count] = A[i];
					solution_count++;
				}
			}
		...
		// Get B arrays
		for (source=nprocs-rem_values; source<nprocs; source++) // Get A arrays
		{
			if (source != serverrank)
			{
				double *A = calloc(size_per_processor, sizeof(double));
				...
				for (i=0; i<size_per_processor;i++)
				{
					//printf("The B message after being recieved is: %f \n", B[i]);
					y[solution_count] = A[i];
					solution_count++;
				}
			}
	...
	while(count<nprocs)
	{
		if( myrank%(2*count)==0)
		{
			if (myrank+count<nprocs)
			{
				...
				//create y of size A+C * double
				//get lengths

				/*
				lengthA = size_per_processor;
				printf("size_per_processor is %d on processor %d \n",size_per_processor, myrank);
				//int lengthC = s;
				length_total = size_per_processor+s;
				printf("s is %d on processor %d\n",s,myrank);
				y = (double *)malloc(length_total*sizeof(double)); 
				//fill y with A to C
				...
				for (i=lengthA; i<length_total; i++)
				{
					y[i]=C[i-lengthA];
				}
				//merge y
				...
				merge(y, l, m, r);
				//copy y back to A
				A = (double *)malloc(length_total*sizeof(double));
...
// Ends here
	stopT = clock();
		...
		//for (i=0; i<length; i++)// Debugg
		//{
		//	printf("The array before the final merge is: %f \n",y[i]);
		//}
		//y[0] = 6;
		//y[1] = 7;
		//y[2] = 8;
		...
		if (myrank == 0)
		{
			for (i=0; i<size_per_processor; i++)// Debugg
			{
				printf("The sorted array is: %f \n",A[i]);
			}
		}
		*/
	...
	if(myrank==0)
	{
		...
		printf("Creating file called results with %dentries run on %d processors\n",size_per_processor,nprocs);
	...
	*/
	/***************************************************************
		Step 4: Server outputs results to a file, named "result"
	***************************************************************/
	if(myrank==0)
	{
		FILE * fout;
